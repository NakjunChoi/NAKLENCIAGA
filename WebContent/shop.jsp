...
<body>
  ...
  <!-- Page Header -->
  <header class="masthead" style="background-image: url('img/market-bg.jpg')">
	...
	<form>
		...
		<a href='addItem.jsp'><input type="button" style="float: right;" class="btn" value="addItem"></a>
		<%} %>
	</form>
	...
	<div class="container">
		<%
			...
			ArrayList<Item> list = itemDAO.getList();
			for(int i=0;i<list.size();i++){
			%>
			<table class='table'>
			<thead>
			<th><%=list.get(i).getItemname() %></th>
			</thead>
			<tr>
			<td rowspan='2'><img width='250'src="<%=list.get(i).getImglink() %>"></td>
			<td rowspan='2'><%=list.get(i).getContent().replaceAll(" ","&nbsp").replaceAll("\n","<br>") %></td>
